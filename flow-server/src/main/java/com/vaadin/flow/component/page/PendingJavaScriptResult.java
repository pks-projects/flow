...
public interface PendingJavaScriptResult extends ExecutionCanceler {
     ...
     *
     * @param targetType
     *            the type to convert the JavaScript return value to
     * @param resultHandler
     *            a handler for the return value from a successful execution
     * @param errorHandler
     *            a handler for an error message in case the execution failed
     */
    <T> void then(Class<T> targetType, SerializableConsumer<T> resultHandler,
    ...
    default <T> CompletableFuture<T> toCompletableFuture(Class<T> targetType) {
        ...
        then(targetType, completableFuture::complete,
                errorValue -> completableFuture.completeExceptionally(
                        new JavaScriptException(errorValue)));
    ...
    default void then(SerializableConsumer<JsonValue> resultHandler,
            SerializableConsumer<String> errorHandler) {
        then(JsonValue.class, resultHandler, errorHandler);
    }
